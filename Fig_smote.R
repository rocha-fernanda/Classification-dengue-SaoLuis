...
# LR
roc(test$CasosBin2, lr_smote_pred[,1], plot=TRUE, legacy.axes=TRUE, percent=TRUE,
...
# LDA
plot.roc(test$CasosBin2, lda_smote_pred[,1], percent=TRUE, col="#4daf4a",
         lwd=3, print.auc=TRUE, add=TRUE, print.auc.x=30, print.auc.y=70, print.auc.cex=1.5)

# Decision Tree
plot.roc(test$CasosBin2, dt_smote_pred[,1], percent=TRUE, col="orangered2",
         lwd=3, print.auc=TRUE, add=TRUE, print.auc.x=30, print.auc.y=64, print.auc.cex=1.5)
# Naive Bayes
plot.roc(test$CasosBin2, nb_smote_pred[,1], percent=TRUE, col="violetred4",
         lwd=3, print.auc=TRUE, add=TRUE, print.auc.x=30, print.auc.y=57, print.auc.cex=1.5)

# Random Forest
plot.roc(test$CasosBin2, rf_smote_pred[,1], percent=TRUE, col="deeppink1",
         lwd=3, print.auc=TRUE, add=TRUE, print.auc.x=30, print.auc.y=44, print.auc.cex=1.5)

title(main="Conjunto SMOTE", font.main= 2, line=2.7, cex.main = 1.6)

legend("bottomright", legend=c("RL_SMOTE", "ADL_SMOTE", "AD_SMOTE", "NB_SMOTE", "FA_SMOTE"),
